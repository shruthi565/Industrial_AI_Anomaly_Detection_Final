...
class Vibration:
    ...
    def create_page(self):
        ...
        self.current_card = self.create_card(cards, "CURRENT VIBRATION", "-- mm/s", "#64748B")
        self.status_card = self.create_card(cards, "VIBRATION STATUS", "WAITING", "#64748B")
        self.create_card(cards, "ANOMALY THRESHOLD", "7 mm/s", "#2563EB")
        ...
        tk.Label(left, text="VIBRATION MONITORING", font=("Arial",18,"bold"), bg="white", fg="#172033").pack(anchor="w", padx=30, pady=(30,25))
        tk.Label(left, text="〰", font=("Arial",52,"bold"), bg="white", fg="#2563EB").pack(pady=(25,5))
        self.big_value = tk.Label(left, text="-- mm/s", font=("Arial",40,"bold"), bg="white", fg="#172033")
        ...
        tk.Label(left, text="Real-time simulated machine vibration monitoring", font=("Arial",10), bg="white", fg="#64748B").pack(pady=(0,25))
        info = tk.Frame(left, bg="#F8FAFC", bd=1, relief="solid")
        info.pack(fill="x", padx=30, pady=15)
        self.value_info = tk.Label(info, text="Current Value                         -- mm/s", font=("Arial",10), bg="#F8FAFC", fg="#64748B", anchor="w")
        self.value_info.pack(fill="x", padx=15, pady=10)
        for text in ["Normal Range                         0–7 mm/s", "Detection Method                    Threshold Monitoring", "Data Type                              Simulated / Arduino Ready"]:
        ...
        tk.Label(right, text="VIBRATION ANALYSIS", font=("Arial",18,"bold"), bg="white", fg="#172033").pack(anchor="w", padx=25, pady=(30,25))
        ...
        self.condition_text = tk.Label(self.condition_box, text="Start Live Monitoring to receive simulated vibration values.", font=("Arial",9), bg="#F0FDF4", fg="#64748B", wraplength=250, justify="left")
    ...
    def read_data(self):
        try:
            with open(STATUS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            value = data.get("vibration")
            if value is None:
                return None
            value = float(value)
            danger = bool(data.get("vibration_danger", value >= self.THRESHOLD))
            return value, danger, data.get("time", "--")
        except (OSError, ValueError, TypeError, json.JSONDecodeError):
            return None

    def refresh_vibration(self):
        ...
        if result is None:
            self.current_card.config(text="-- mm/s", fg="#64748B")
            self.status_card.config(text="WAITING", fg="#64748B")
            self.big_value.config(text="-- mm/s", fg="#172033")
            ...
            self.current_card.config(text=f"{value:.2f} mm/s", fg=color)
            self.status_card.config(text="HIGH" if danger else "NORMAL", fg=color)
            self.big_value.config(text=f"{value:.2f} mm/s", fg=color if danger else "#172033")
            self.big_status.config(text="HIGH VIBRATION" if danger else "NORMAL", fg=color)
            self.value_info.config(text=f"Current Value                         {value:.2f} mm/s")
            self.time_label.config(text=f"Last update: {update_time}")
            if danger:
                self.condition_box.config(bg="#FEF2F2")
                self.condition_title.config(text="HIGH VIBRATION", bg="#FEF2F2", fg="#DC2626")
                self.condition_text.config(text="Machine vibration has reached or exceeded the anomaly threshold.", bg="#FEF2F2")
                self.detection.config(text="⚠ Vibration anomaly detected.", fg="#DC2626")
            else:
                self.condition_box.config(bg="#F0FDF4")
                self.condition_title.config(text="NORMAL CONDITION", bg="#F0FDF4", fg="#16A34A")
                self.condition_text.config(text="Machine vibration is within the expected range.", bg="#F0FDF4")
                self.detection.config(text="Monitoring active. No vibration anomaly detected.", fg="#64748B")
        self.after_id = self.parent.after(1000, self.refresh_vibration)
